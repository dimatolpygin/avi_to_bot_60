...
# Читаем оригинал файла заказчика, а не копию: копия молча устареет, и бот
# начнёт называть вчерашние цены. Папка `материалы/` в git не идёт (клиентские
# данные), в контейнер монтируется только на чтение — см. docker-compose.yml.
FAYL_PO_UMOLCHANIYU = "материалы/Копия ИП Семенко ЕВ.xlsx"
...
# Наличие: как читать колонку, если заказчик её заведёт. Сейчас её в файле нет,
# и все позиции получают `unknown` — бот отвечает «уточню у менеджера».
# Выдумывать наличие нельзя: это ровно тот баг, из-за которого бота переделывают.
...
def _nalichie(syroe: str | None) -> str:
    ...
    if z in _EST:
        return "in_stock"
    if z in _NET or z == "":     # пусто = «нет» — так заказчик ведёт колонку
        return "out"
