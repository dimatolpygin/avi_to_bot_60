...
_SEMEYSTVA = {
    ...
    # «Электрокаменка …» (11 ЭКМ мягкого ассортимента, курс 31.07) — это печь:
    # свод (slovar_svodnyy.json) уже держит «электрокаменка» синонимом печи, но
    # ETL клал им family «электрокаменка» (первое слово), и канал «семейство» не
    # связывал их с запросом «печь». Схлопываем в «печь» здесь же — заодно
    # разбираются мощность и объём (парсятся только у family == «печь»).
    "электрокаменка": "печь",
}
