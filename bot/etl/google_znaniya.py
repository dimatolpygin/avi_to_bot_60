...
# СЛУЖЕБНЫЕ (code-owned) блоки услуг: механика и предохранители, а НЕ факты. Их
# канонический текст — в `bot/znaniya.py`; во вкладке они стоят заглушкой
# «[Служебный блок: …]», чтобы заказчик их не правил. Синк такие ключи НЕ вставляет
# и НЕ обновляет из вкладки (иначе заглушка затёрла бы, например, запрет боту
# называть выдуманные цены — `chego_ne_znaesh`). У товарного аккаунта механика
# живёт в скелете `znaniya_tovar.SISTEMNY_SHABLON`, во вкладке её нет — этих
# ключей там не бывает, поэтому набор общий и товарного не касается.
SLUZHEBNYE_KLYUCHI = frozenset({"chego_ne_znaesh", "kak_rabotat", "vedenie"})
