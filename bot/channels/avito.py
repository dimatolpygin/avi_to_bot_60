...
def sdelat_obrabotchik(kod: str, api: AvitoAPI, yadro,
    ...
    async def obrabotchik(v: Vhodyashchee) -> None:
        ...
        if v.tekst is None:
            ...
            # фото уже ушло в amoCRM/журнал (выше), но дежурную просьбу «опишите
            # словами» не шлём (это ответ бота).
            if not yadro.otvechaet(kod):
                logger.info("🔌 Авито «%s»: вложение в чате %s, но бот выключен "
                            "рубильником — молчу (фото в amoCRM ушло)", kod, v.chat_id)
                return
