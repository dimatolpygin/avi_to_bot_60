# -*- coding: utf-8 -*-
"""Фоновая синхронизация БАЗЫ ЗНАНИЙ услуг из Google-таблицы (этап 19, Шаг 1).

Аналог `sinhronizatsiya.py` (каталог), но для аккаунтов услуг: раз в
`GOOGLE_SYNC_INTERVAL_S` секунд читаем вкладку знаний каждого сервисного аккаунта
и обновляем `knowledge_blocks`; горячую перезагрузку промпта услуг в памяти ядра
подключает колбэк `posle_zapisi` (`Yadro.perezagruzit_prompt_uslug`).
