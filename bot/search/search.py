...
def cena_za_metr_kvadratnyy(p: Pozitsiya) -> Decimal | None:
    """Цена за м² — ТОЛЬКО там, где у позиции есть рабочая ширина.

    Рабочая ширина берётся из скобок в названии (`15х95 (88)`). У полка скобок
    нет: 90 это полная ширина, полок кладут с зазорами, рабочей ширины у него
    не существует. Посчитать полок «как вагонку» значит выдать бессмысленную
    цифру, поэтому пустое поле здесь — запрет на пересчёт, а не «посчитаем как-нибудь».
    """
    if p.working_width_mm is None or p.price_per_m is None:
        return None
    metrov_v_kvadrate = Decimal(1000) / Decimal(p.working_width_mm)
