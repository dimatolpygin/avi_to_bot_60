...
class Yadro:
    ...
    def __init__(self, cfg: Config, pamyat: Pamyat, *, tempo: Tempo | None = None,
        ...
        # Короткая метка ««заголовок» за цена» того же объявления — уходит в
        # РЕЗУЛЬТАТ ПОИСКА (не только в системный факт): напоминание рядом с
    ...
    def _otvetchik(self, prof: Profil):
        ...
        async def otvechat(vopros: str, istoriya: list[dict], klyuch: str) -> str:
            rezultat = await otvetit(
                ...
                # Пустая выдача под активным объявлением → товар в продаже, не
                # отшиваем «нет в прайсе»/«уточню у поставщика» (этап 14).
                pod_obyavleniem=bool(self._obyavleniya.get(klyuch)),
                # Метка объявления в результат поиска: каталог ≠ товар объявления,
                # его цену каталогом не перебиваем.
                obyavlenie_kratko=self._obyavleniya_metka.get(klyuch, ""),
            )
    ...
    def _sistemny(self, kod: str, klyuch: str) -> str | None:
        ...
        # Дата/время добавляем ВСЕГДА (не зависят от чата) — чтобы бот знал текущий
        # день недели и не путал «сегодня»/«завтра»/выходной; объявление — по ключу.
        chasti = [c for c in (baza, _fakt_daty(), self._obyavleniya.get(klyuch)) if c]
        return "\n\n".join(chasti) if chasti else None
    ...
    def zapomnit_obyavlenie(self, kod: str, chat: str | int, obyavlenie: dict | None) -> None:
        ...
        if fakt:
            ...
            self._obyavleniya_metka.pop(klyuch, None)
...
def _metka_obyavleniya(o: dict) -> str:
    """Короткая метка объявления для результата поиска: ««заголовок» за цена».

    Пустой заголовок → пусто (метку в поиск не кладём). Уходит в `payload_poiska`
    рядом с каталожными ценами — чтобы модель не выдавала каталожную позицию за
    товар объявления и не оспаривала его цену (системного факта ей мало против
    конкретных цифр из поиска)."""
...
def _fakt_obyavleniya(o: dict, opisanie: str = "") -> str:
    ...
    128 объявлений против ~200 строк каталога — гималайская соль, трубы дымохода
    и пр. рекламируются, но в базе поиска их нет). Раз объявление размещено —
    товар РЕАЛЬНО в продаже, поэтому активное объявление ПЕРЕБИВАЕТ пустую выдачу
    поиска: под ним бот не говорит «нет в прайсе/базе» и не обещает «уточню у
    поставщика» (это ровно то, на что жалуется заказчик). Оговорки — те же, что
    всюду: объёмы по объявлению не считаем, точные цену/характеристики сверх
    объявления не выдумываем, гейт «не наш профиль» остаётся выше.
    """
    title = (o.get("title") or "").strip()
    if not title:
        return ""
    ...
    return (
        ...
        f"явно не спросил про другое. На короткий или общий вопрос («что за товар?», «это "
        f"что?», «расскажите», «актуально?», «сколько стоит?», «в наличии?», «подробнее») "
        f"НЕ отвечай общим «что вас интересует?» и НЕ перечисляй весь ассортимент — это "
        f"провал: клиент уже показал, что смотрит. Сразу назови, что это за товар (по "
        f"заголовку объявления) и его цену из объявления, и предложи следующий шаг "
        f"(оформить, замер, оставить контакт). ЦЕНА ИЗ ОБЪЯВЛЕНИЯ — это действительная "
        f"цена этого товара для клиента. Даже если в каталоге/поиске есть похожий товар "
        f"с ДРУГОЙ ценой — НЕ противоречь объявлению: не говори, что цена устарела, «за "
        f"прошлый период» или «за другой формат», и НЕ называй вместо неё цену из "
        f"каталога. Клиент видит это объявление — для него верна его цена. НЕ подменяй "
        f"товар из объявления другим товаром из каталога — окно это не дверь, соль это не "
        f"камень: товар объявления в продаже, даже если поиск по прайсу его не нашёл (в "
        f"каталог занесены не все объявления аккаунта). Поэтому не говори «нет в прайсе» "
        f"или «нет в базе» и не обещай «уточню у поставщика», и замену из каталога не "
        f"подставляй. Если клиент спрашивает деталь, которой в объявлении (заголовок и "
        f"цена) НЕТ — точный размер, объём, состав, срок — не выдумывай и не бери из "
...
_DNI_NEDELI = ("понедельник", "вторник", "среда", "четверг", "пятница",
               "суббота", "воскресенье")
_MESYATSY = ("", "января", "февраля", "марта", "апреля", "мая", "июня", "июля",
             "августа", "сентября", "октября", "ноября", "декабря")


def _rabochiy_den(wd: int) -> str:
    """Часы работы по дню недели (0=пн). Пн–пт 9–18, сб 10–16, вс выходной."""
    if wd <= 4:
        return "рабочий день, с 9:00 до 18:00"
    if wd == 5:
        return "рабочий день, с 10:00 до 16:00 (суббота, короткий день)"
    return "выходной, не работаем"


def _fakt_daty() -> str:
    """Текущие дата, время и день недели по Краснодару для системного промпта.

    Краснодар живёт по МСК (UTC+3, без перехода на летнее время), поэтому берём
    фиксированный сдвиг, а не `zoneinfo`: не тянем пакет `tzdata` и не зависим от
    таймзоны хоста/контейнера. Подмешивается в промпт КАЖДЫЙ раз (в `_sistemny`),
    иначе модель не знает текущий день и выдумывает, работаем ли «сегодня»/«завтра»
    (баг 13.09: бот сказал «завтра выходной», не зная реального дня недели).
    Рабочие ли сегодня и завтра — считаем в коде, чтобы модель не сопоставляла день
    недели с часами в уме и не ошибалась.
    """
    from datetime import datetime, timedelta, timezone
    now = datetime.now(timezone(timedelta(hours=3)))
    zavtra = now + timedelta(days=1)
    return (
        f"СЕЙЧАС (время Краснодара, это московское время): {_DNI_NEDELI[now.weekday()]}, "
        f"{now.day} {_MESYATSY[now.month]} {now.year} года, {now:%H:%M}. "
        f"Сегодня — {_rabochiy_den(now.weekday())}. "
        f"Завтра — {_DNI_NEDELI[zavtra.weekday()]}, {_rabochiy_den(zavtra.weekday())}. "
        "Про «сегодня», «завтра», «в выходные», «когда вы работаете», «можно ли подъехать» "
        "отвечай СТРОГО по этой дате и этим часам: день недели не выдумывай и не считай в уме. "
