...
def _fakt_obyavleniya(o: dict) -> str:
    ...
    return (
        f"ФАКТ: клиент пишет под нашим объявлением «{title}»{hvost}. Объявление "
        f"размещено — значит этот товар у нас в продаже, даже если поиск по прайсу его "
        f"не нашёл: в прайс-каталог занесены не все объявления аккаунта. Если вопрос про "
        f"это объявление, а поиск вернул пусто — НЕ говори «такого нет», «нет в прайсе» "
        f"или «нет в базе» и НЕ обещай «уточню у поставщика»: товар есть, объявление "
        f"активно. Подтверди наличие, ответь как продавец (что это и цена из объявления) "
        f"и веди к заказу или контакту. Это подсказка о том, что смотрит клиент, а не "
        f"прайс: объёмы по объявлению не считай, а точные цену и характеристики, которых "
        f"в объявлении нет, не выдумывай — предложи уточнить. Гейт «не наш профиль» "
        f"остаётся выше: спрашивает про постороннее, не про это объявление — отвечай как обычно."
    )
