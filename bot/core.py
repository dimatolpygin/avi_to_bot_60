...
def _fakt_obyavleniya(o: dict) -> str:
    ...
    return (
        ...
        f"(оформить, замер, оставить контакт). НЕ подменяй товар из объявления другим "
        f"товаром из каталога — окно это не дверь, соль это не камень: если товара из "
        f"объявления в поиске нет, это нормально, отвечай по объявлению и замену не "
        f"подставляй. Объявление активно — значит товар в "
        f"продаже, даже если поиск по прайсу его не нашёл (в каталог занесены не все "
        f"объявления аккаунта): не говори «нет в прайсе» или «нет в базе» и не обещай "
        f"«уточню у поставщика». Оговорки: объёмы по объявлению не считай; точные цену и "
        f"характеристики, которых в объявлении нет, не выдумывай — предложи уточнить; гейт "
        f"«не наш профиль» остаётся выше (постороннее, не про это объявление и не банное — "
        f"отвечай как обычно)."
    )
